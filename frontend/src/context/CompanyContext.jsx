...
import { isPortfolioRole, useAuth } from './AuthContext';
...
export const CompanyProvider = ({ children }) => {
  ...
  const portfolioUser = isPortfolioRole(user);

  const switchCompany = useCallback(
    (company) => {
      setActiveCompany(company);
      if (portfolioUser && company?.slug) {
  ...
  const refreshCompanies = useCallback(async () => {
    ...
    try {
      ...
      setActiveCompany((current) => {
        if (isPortfolioRole(user)) {
          // URL sync owns selection for admin/MD; keep current if already set
