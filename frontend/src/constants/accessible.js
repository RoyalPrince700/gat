export const ACCESSIBLE_SLUG = 'accessible-publishers';

export const ACCESSIBLE_SEASONS = ['2023-2024', '2024-2025', '2025-2026'];

/** Infer canonical season from a filename like "2023-2024 SEASON.xlsx". */
export const inferSeasonFromFilename = (filename) => {
  const match = String(filename || '').match(/(20\d{2})\s*[-_/]\s*(20\d{2})/);
  if (!match) return '';
  const key = `${match[1]}-${match[2]}`;
  return ACCESSIBLE_SEASONS.includes(key) ? key : '';
};

/**
 * Loyalty / segmentation defaults for school book-purchase analysis.
 * Keep in sync with backend/utils/accessibleLoyalty.js.
 * Snapshot comments (clean all-seasons, Aug 2026 — UI must use the API):
 * 2,151 schools, ₦1,260,066,623; median ₦260,000; avg ₦585,805.
 */
export const ACCESSIBLE_NAIRA_PER_POINT = 1000;
export const ACCESSIBLE_GIFT_COST_PER_POINT = 30;
export const ACCESSIBLE_GIFT_COST_RANGE = [20, 50];

export const ACCESSIBLE_SPEND_SEGMENTS = [
  {
    id: 'occasional',
    label: 'Occasional',
    min: 0,
    max: 50000,
    tier: 'bronze',
    loyaltyRole: 'Low earn; keep cost of rewards tiny',
  },
  {
    id: 'small',
    label: 'Small',
    min: 50000,
    max: 200000,
    tier: 'bronze',
    loyaltyRole: 'Entry tier',
  },
  {
    id: 'core',
    label: 'Core',
    min: 200000,
    max: 500000,
    tier: 'silver',
    loyaltyRole: 'Volume of customers; median sits here',
  },
  {
    id: 'strong',
    label: 'Strong',
    min: 500000,
    max: 1000000,
    tier: 'gold',
    loyaltyRole: 'Solid repeat / grow into key',
  },
  {
    id: 'key',
    label: 'Key account',
    min: 1000000,
    max: 5000000,
    tier: 'platinum',
    loyaltyRole: 'Half of revenue — protect and reward',
  },
  {
    id: 'strategic',
    label: 'Strategic',
    min: 5000000,
    max: null,
    tier: 'diamond',
    loyaltyRole: 'White-glove; not the same gift as ₦50k buyers',
  },
];

export const ACCESSIBLE_LOYALTY_TIERS = [
  { id: 'bronze', label: 'Bronze', from: 'Occasional + Small (< ₦200k)' },
  { id: 'silver', label: 'Silver', from: 'Core (₦200k–500k)' },
  { id: 'gold', label: 'Gold', from: 'Strong (₦500k–1m)' },
  { id: 'platinum', label: 'Platinum', from: 'Key (₦1m–5m)' },
  { id: 'diamond', label: 'Diamond', from: 'Strategic (₦5m+)' },
];

export const ACCESSIBLE_GIFT_LADDER = [
  {
    points: 50,
    label: 'Jotter & pen',
    note: '~₦50k spend',
    examples: [
      'Branded jotter + pen set',
      'Exercise-book pack',
      'Desk notepad',
    ],
  },
  {
    points: 250,
    label: 'Flask / umbrella',
    note: '~median school',
    examples: [
      'Lunch flask',
      'Umbrella',
      '10,000mAh power bank',
      'Quality diary + pen',
    ],
  },
  {
    points: 500,
    label: 'Kettle / backpack',
    note: '~₦500k spend',
    examples: [
      'Electric kettle',
      'Sandwich maker',
      'Rechargeable fan',
      'School backpack',
    ],
  },
  {
    points: 1000,
    label: 'Blender / fan',
    note: '~₦1m spend',
    examples: [
      'Table blender (Binatone / Scanfrost class)',
      'Standing fan',
      'Rice cooker',
      'Pressing iron + kettle',
    ],
  },
  {
    points: 5000,
    label: 'Phone / TV',
    note: '~₦5m spend',
    examples: [
      'Android phone (Infinix / Tecno range)',
      '32-inch LED TV',
      'Small fridge',
      'Microwave',
    ],
  },
];

export const spendToPoints = (amount, nairaPerPoint = ACCESSIBLE_NAIRA_PER_POINT) => {
  const rate = Number(nairaPerPoint) > 0 ? Number(nairaPerPoint) : ACCESSIBLE_NAIRA_PER_POINT;
  return Math.floor(Math.max(0, Number(amount) || 0) / rate);
};

export const ACCESSIBLE_CATEGORIES = [
  { value: 'physical_print', label: 'Physical print' },
  { value: 'audio_books', label: 'Audio books' },
  { value: 'ebooks', label: 'E-books' },
  { value: 'content_animations', label: 'Content animations' },
  { value: 'braille', label: 'Braille' },
  { value: 'printing_services', label: 'Printing services' },
  { value: 'procurement', label: 'Procurement' },
  { value: 'other', label: 'Other' },
];

export const ACCESSIBLE_LEVELS = [
  { value: 'elementary', label: 'Elementary' },
  { value: 'secondary', label: 'Secondary' },
  { value: 'tertiary', label: 'Tertiary' },
  { value: 'general', label: 'General' },
];

export const accessibleCategoryLabel = (value) =>
  ACCESSIBLE_CATEGORIES.find((c) => c.value === value)?.label || value || '—';

export const accessibleLevelLabel = (value) =>
  ACCESSIBLE_LEVELS.find((l) => l.value === value)?.label || value || '—';
